(() => {
  ...
  const defaultState = () => ({
    ...
    activeTab:'status',history:{summary:'',goals:'',relationships:'',milestones:'',origin:'',age:'',affiliation:'',description:'',tagline:'',birth:'',residence:'',portraitUrl:'',bannerUrl:'',bannerPositionY:50,bannerScale:100},appearance:{mode:'dark',palette:'red'},fortuneBlessing:{enabled:false,tempHp:0},notes:'',
  ...
  function migrate(data){
    ...
    out.appearance=Object.assign({mode:'dark',palette:'red'},data?.appearance||{});
  ...
  function syncShellChrome(){
    ...
    document.body.dataset.theme=mode==='light'?'light':'standard';
    document.body.dataset.mode=mode;
    document.body.dataset.palette=palette;
    document.documentElement.style.colorScheme=mode==='light'?'light':'dark';
    if(modeBtn){
      modeBtn.classList.toggle('is-light',mode==='light');
      modeBtn.setAttribute('aria-pressed',mode==='light'?'true':'false');
      modeBtn.setAttribute('title',mode==='light'?'Mudar para Dark':'Mudar para Light');
    ...
    if(paletteBtn)paletteBtn.setAttribute('aria-label',`Tema atual: ${THEME_PALETTES[palette].label}`);
    ...
    document.querySelectorAll('[data-palette-choice]').forEach(btn=>{const selected=btn.dataset.paletteChoice===palette;btn.classList.toggle('active',selected);btn.setAttribute('aria-selected',selected?'true':'false')});
    if(bg){
  ...
  function bindShellChrome(){
    const bannerBtn=byId('bannerBtn'),clear=byId('clearBannerBtn'),help=byId('helpBtn'),helpDialog=byId('helpDialog'),posInput=byId('bannerPositionInput'),scaleInput=byId('bannerScaleInput'),modeBtn=byId('themeModeBtn'),paletteBtn=byId('themePaletteBtn'),themeDialog=byId('themeDialog');
    ...
    if(modeBtn)modeBtn.onclick=()=>{state.appearance=state.appearance||{mode:'dark',palette:'red'};state.appearance.mode=state.appearance.mode==='light'?'dark':'light';save();syncShellChrome();notify(state.appearance.mode==='light'?'Modo Light ativado.':'Modo Dark ativado.');};
    if(paletteBtn)paletteBtn.onclick=()=>{syncShellChrome();if(themeDialog?.showModal)themeDialog.showModal();};
    document.querySelectorAll('[data-palette-choice]').forEach(btn=>btn.onclick=()=>{const palette=btn.dataset.paletteChoice;if(!THEME_PALETTES[palette])return;state.appearance=state.appearance||{mode:'dark',palette:'red'};state.appearance.palette=palette;save();syncShellChrome();notify(`Tema ${THEME_PALETTES[palette].label} ativado.`);});
  }
