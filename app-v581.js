(() => {
  ...
  function renderFortuneBlessingCompact(context='status'){
    ...
    return `<div class="fortune-temp-hp ${f.enabled?'active':''} fortune-${context}"><label><input type="checkbox" data-fortune-toggle ${f.enabled?'checked':''}><span>Bênção de Fortuna</span></label><small>${f.enabled?`HP temporário <b>${temp}/10</b>`:'+10 HP temporários mensais'}</small></div>`;
  }
  ...
  function renderCombatResourceDock(){
    ...
    const mini=(kind,label,current,max,extra='')=>{const inputId=`combat-${kind}Manual`,ratio=max?clamp((Number(current)||0)/Number(max)*100,0,100):0;return `<div class="combat-resource-chip resource-${kind}"><span>${label}</span><strong>${current}<small>/${max}</small></strong><div class="combat-mini-meter"><i style="width:${ratio}%"></i></div>${extra}<div class="combat-resource-adjust"><input id="${inputId}" data-resource-manual-input="${kind}" type="number" step="1" placeholder="+/−"><button type="button" data-manual-resource="${kind}" data-manual-input="${inputId}" title="Aplicar">↵</button></div></div>`};
