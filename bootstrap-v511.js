(async () => {
  ...
  try {
    await new Promise((resolve,reject)=>{
      ...
      patch.src=`lineage-creation-v515.js?v=v5.15.11-lineage-semantics-${encodeURIComponent(window.DUODECIMA_CORE_STATE?.version || 'fallback')}`;
  ...
  script.src = `app-v511.js?v=v5.15.11-lineage-semantics-${encodeURIComponent(window.DUODECIMA_CORE_STATE?.version || 'fallback')}`;
  script.defer = false;
  script.onload=async()=>{
    try{
      await new Promise((resolve,reject)=>{
