(async () => {
  ...
  try {
    await new Promise((resolve,reject)=>{
      ...
      patch.src=`lineage-creation-v515.js?v=v5.15.12-familiar-sheet-${encodeURIComponent(window.DUODECIMA_CORE_STATE?.version || 'fallback')}`;
  ...
  script.src = `app-v511.js?v=v5.15.12-familiar-sheet-${encodeURIComponent(window.DUODECIMA_CORE_STATE?.version || 'fallback')}`;
  script.defer = false;
  script.onload=async()=>{
    try{
      await new Promise((resolve,reject)=>{
