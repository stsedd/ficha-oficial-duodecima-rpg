(() => {
  ...
  const defaultState = () => ({
    ...
    activeTab:'status',history:{summary:'',goals:'',relationships:'',milestones:'',origin:'',age:'',affiliation:'',description:'',tagline:'',birth:'',residence:'',portraitUrl:'',bannerUrl:'',bannerPositionY:50,bannerScale:100},appearance:{mode:'dark'},fortuneBlessing:{enabled:false,tempHp:0},notes:'',
  ...
  function migrate(data){
    ...
    out.appearance=Object.assign({mode:'dark'},data?.appearance||{});
  ...
  function syncShellChrome(){
    const h=state.history||{}, banner=String(h.bannerUrl||''), pos=clamp(Number(h.bannerPositionY)||50,0,100), zoom=clamp(Number(h.bannerScale)||100,100,170), bg=byId('siteBannerBg'), empty=byId('siteBanner')?.querySelector('.profile-cover-empty'), wrap=byId('bannerMiniWrap'), mini=byId('bannerMini'), clear=byId('clearBannerBtn'), name=byId('bannerCharacterName'), bannerBtn=byId('bannerBtn'), controls=byId('bannerControls'), posInput=byId('bannerPositionInput'), scaleInput=byId('bannerScaleInput'), posValue=byId('bannerPositionValue'), scaleValue=byId('bannerScaleValue'), themeBtn=byId('themeToggleBtn');
    const theme=state.appearance?.mode==='light'?'light':'dark';
    document.body.dataset.theme=theme==='light'?'light':'standard';
    document.documentElement.style.colorScheme=theme==='light'?'light':'dark';
    if(themeBtn){
      themeBtn.classList.toggle('is-light',theme==='light');
      themeBtn.setAttribute('aria-pressed',theme==='light'?'true':'false');
      themeBtn.setAttribute('title',theme==='light'?'Mudar para tema escuro':'Mudar para tema claro');
      const icon=themeBtn.querySelector('.theme-toggle-icon'),label=themeBtn.querySelector('.theme-toggle-copy b');
  ...
  function bindShellChrome(){
    const bannerBtn=byId('bannerBtn'),clear=byId('clearBannerBtn'),help=byId('helpBtn'),dialog=byId('helpDialog'),posInput=byId('bannerPositionInput'),scaleInput=byId('bannerScaleInput'),themeBtn=byId('themeToggleBtn');
    ...
    if(themeBtn)themeBtn.onclick=()=>{state.appearance=state.appearance||{mode:'dark'};state.appearance.mode=state.appearance.mode==='light'?'dark':'light';save();syncShellChrome();notify(state.appearance.mode==='light'?'Tema claro ativado.':'Tema escuro ativado.');};
  }
