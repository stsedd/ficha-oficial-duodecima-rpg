(async () => {
  ...
  const THEME_PALETTES = {
    red:{label:'Vermelho'}, pink:{label:'Rosa'}, blue:{label:'Azul'}, orange:{label:'Laranja'}, green:{label:'Verde'}, purple:{label:'Roxo'},
    gold:{label:'Dourado'}, silver:{label:'Prata'}, wine:{label:'Vinho'}, teal:{label:'Teal'}, sepia:{label:'Sépia'},
    blush:{label:'Blush'}, peach:{label:'Peach'}, lilac:{label:'Lilac'}, sage:{label:'Sage'}, lemon:{label:'Lemon'},
    mauve:{label:'Mauve'}, coral:{label:'Coral'}, aqua:{label:'Aqua'}, sand:{label:'Sand'}, sky:{label:'Sky'},
  ...
  const magicRules = window.DUODECIMA_MAGIC || {circles:[],castingAttributes:['int','fe','car'],maxSacrifices:3,sacrificeEnergyEach:25,hpProgressionPenalty:2,highCircleUses:{6:2,7:2,8:1,9:1},sacrificeCanGoBelowZero:true};
  const lineageRules = window.DUODECIMA_LINEAGE || system.lineage || {
    normal:{label:'Prole direta',formula:'DEUS',description:'É filho direto de uma única divindade e usa somente o kit dessa origem.'},
    compound:{label:'Legado Composto',formula:'DEUS + LEGADO',description:'É filho direto de um deus, mas também carrega uma herança divina secundária.'},
    direct:{label:'Legado Direto',formula:'LEGADO + LEGADO',description:'É filho de semideuses e carrega duas heranças divinas próximas.'}
  };
  ...
  function exclusiveThemeAssetOk(value){return typeof value==='string'&&/^data:(?:image\/(?:png|jpeg|jpg|webp|gif|svg\+xml)|font\/(?:woff2?|ttf|otf)|application\/(?:font-woff|font-woff2|x-font-ttf|x-font-opentype));base64,[A-Za-z0-9+/=\s]+$/i.test(value)}
  ...
  const defaultFamiliarAbility = (type='passiva') => ({id:uid('famab'),name:'Nova habilidade',type,description:''});
...
function resourceByKey(key){return divineResources().find(r=>resourceKey(r)===key)||null}
  ...
  function migrate(data){
    ...
    out.weapons=Array.isArray(data?.weapons)?data.weapons.map(w=>Object.assign({id:uid('weapon'),name:'Arma',type:'corpo-a-corpo',attr:'for',material:'ferro-aco',stakes:0,resistanceCurrent:3,attackExtra:0,damageExtra:0,equipped:true,imageUrl:'',isHeritage:false,notes:''},w,{isHeritage:!!w?.isHeritage})):[];
    ...
    const legacyTheme=data?.theme||'';
    const legacyAppearance=legacyTheme==='parchment'?{mode:'light',palette:'sepia',special:'none'}:legacyTheme==='obsidian'?{mode:'dark',palette:'blue',special:'none'}:legacyTheme==='emerald'?{mode:'dark',palette:'green',special:'none'}:{mode:'dark',palette:'red',special:'none'};
    out.appearance=Object.assign(legacyAppearance,data?.appearance||{});
    ...
    out.history.bannerScale=clamp(Number(out.history.bannerScale)||100,100,220);
  ...
  function renderCreationLineage(){
    ...
    const normalRule=lineageRules.normal||{},compoundRule=lineageRules.compound||{},directRule=lineageRules.direct||{};
    return `<div class="legacy-creation-box">
      <div class="section-title"><div><p class="eyebrow">LINHAGEM</p><h3>Origem divina do personagem</h3></div>${type!=='normal'?'<span class="pill good">2 origens</span>':'<span class="pill">1 origem</span>'}</div>
      <div class="lineage-definition-grid">
  ...
  function renderTabs(){
    ...
    return `<nav class="sheet-tabs" data-theme-component="tabs" aria-label="Seções da ficha"><div class="sheet-tab-scroll">${tabs.map(([id,label],idx)=>`<button type="button" data-tab="${id}" data-theme-tab="${id}" aria-current="${state.activeTab===id?'page':'false'}" class="${state.activeTab===id?'active':''}"><span>${String(idx+1).padStart(2,'0')}</span>${label}</button>`).join('')}</div></nav>`;
  }
  function resourceAdjustBox(kind,label,current,max,quick,context='status',showBand=false){
    ...
    return `<div class="resource-chip resource-${kind}" ${themeResourceAttrs(kind)} data-theme-frame="resource"><div class="row between"><span>${themeIconSlot(kind)}<span class="theme-resource-label">${label}</span></span><strong>${current}/${max}</strong></div><div class="resource-meter"><i style="width:${ratio}%"></i></div><div class="mini-actions">${quickButtons}${full}</div><div class="resource-manual"><input id="${inputId}" data-resource-manual-input="${kind}" type="number" step="1" placeholder="Ex.: -37 ou 20"><button type="button" data-manual-resource="${kind}" data-manual-input="${inputId}">Aplicar</button></div>${band?`<div class="status-line ${band.cls}"><b>${band.name}</b><span>${band.text}</span></div>`:''}</div>`;
  }
  function renderDivineResourceCore(r,context='status'){
    ...
    return `<div class="core-stat core-resource divine-resource-card" ${themeResourceAttrs('divine',r.id||'',key)} data-resource-scope="${esc(r.scope||'personal')}" data-theme-frame="resource"><div class="core-stat-label">${themeIconSlot('divine')}<span class="theme-resource-label">${esc(r.name)}</span> <small>${scopeLabel}</small></div><div class="core-stat-value">${current}<small>/${max}</small></div><div class="core-stat-meter"><i style="width:${ratio}%"></i></div>${r.thresholds?.length?`<div class="core-stat-caption">limiares ${r.thresholds.join(' · ')}</div>`:''}<div class="core-stat-adjust"><input id="${inputId}" data-divine-resource-input="${esc(key)}" type="number" step="1" placeholder="+/−"><button type="button" data-divine-resource-apply="${esc(key)}" data-manual-input="${inputId}" title="Aplicar">↵</button></div></div>`;
  }
  function renderResourceDock(){
    const band=sanityBand();
    const core=(kind,label,current,max,extra='')=>{const inputId=`status-${kind}Manual`,ratio=max?clamp((Number(current)||0)/Number(max)*100,0,100):0;return `<div class="core-stat core-${kind}" ${themeResourceAttrs(kind)} data-theme-frame="resource"><div class="core-stat-label">${themeIconSlot(kind)}<span class="theme-resource-label">${label}</span></div><div class="core-stat-value">${current}<small>/${max}</small></div><div class="core-stat-meter"><i style="width:${ratio}%"></i></div>${extra}<div class="core-stat-adjust"><input id="${inputId}" data-resource-manual-input="${kind}" type="number" step="1" placeholder="+/−"><button type="button" data-manual-resource="${kind}" data-manual-input="${inputId}" title="Aplicar">↵</button></div></div>`};
    return `<section class="status-core-strip" data-theme-component="resource-strip">
      ${core('hp','HP',state.currentHp,hpMax())}
      ${core('san','Sanidade',state.currentSanity,100,`<div class="core-stat-caption">${band.name}</div>`)}
      <div class="core-stat core-defense" ${themeResourceAttrs('defense')} data-theme-frame="resource"><div class="core-stat-label">${themeIconSlot('defense')}<span class="theme-resource-label">Defesa</span></div><div class="core-stat-value">${signed(defenseBonus())}</div><div class="core-stat-caption">1d20 ${signed(defenseBonus())}</div><div class="core-stat-caption muted-mini">DES + equipamento</div></div>
      ${core('en','Energia',state.currentEnergy,energyMax())}
      ${hudResources().map(r=>renderDivineResourceCore(r,'status')).join('')}
      <div class="core-stat core-cast" ${themeResourceAttrs('cast')} data-theme-frame="resource"><div class="core-stat-label">${themeIconSlot('cast')}<span class="theme-resource-label">Conjuração</span></div><div class="core-stat-value">${signed(castAttack())}</div><div class="core-stat-caption">DT ${castDT()} · ${attrName(god().casting)}</div></div>
    </section>`;
  }
  function renderCombatResourceDock(){
    const g=god(),fDef=fixedDefense();
    const mini=(kind,label,current,max)=>{const inputId=`combat-${kind}Manual`,ratio=max?clamp((Number(current)||0)/Number(max)*100,0,100):0;return `<div class="combat-resource-chip resource-${kind}" ${themeResourceAttrs(kind)} data-theme-frame="resource"><span>${themeIconSlot(kind)}<span class="theme-resource-label">${label}</span></span><strong>${current}<small>/${max}</small></strong><div class="combat-mini-meter"><i style="width:${ratio}%"></i></div><div class="combat-resource-adjust"><input id="${inputId}" data-resource-manual-input="${kind}" type="number" step="1" placeholder="+/−"><button type="button" data-manual-resource="${kind}" data-manual-input="${inputId}" title="Aplicar">↵</button></div></div>`};
    const divine=r=>{const current=resourceValue(r),max=resourceMaxFor(r),ratio=max?clamp(current/max*100,0,100):0,key=resourceKey(r),inputId=`combat-divine-${String(r.id).replace(/[^a-z0-9_-]/gi,'-')}`;return `<div class="combat-resource-chip resource-divine" ${themeResourceAttrs('divine',r.id||'',key)} data-theme-frame="resource"><span>${themeIconSlot('divine')}<span class="theme-resource-label">${esc(r.name)}</span> ${r.scope==='collective'?'<em>COLETIVO</em>':''}</span><strong>${current}<small>/${max}</small></strong><div class="combat-mini-meter"><i style="width:${ratio}%"></i></div><div class="combat-resource-adjust"><input id="${inputId}" data-divine-resource-input="${esc(key)}" type="number" step="1" placeholder="+/−"><button type="button" data-divine-resource-apply="${esc(key)}" data-manual-input="${inputId}">↵</button></div></div>`};
    const readOnly=(kind,label,value,caption='')=>{const resourceKind=kind==='dock-defense'?'defense':kind==='dock-dt'?'dt':kind;return `<div class="combat-resource-chip combat-readonly ${kind}" ${themeResourceAttrs(resourceKind)} data-theme-frame="resource"><span>${themeIconSlot(resourceKind)}<span class="theme-resource-label">${label}</span></span><strong>${value}</strong>${caption?`<small>${caption}</small>`:''}</div>`};
    return `<section class="combat-resource-dock" data-theme-component="combat-resource-dock" aria-label="Recursos de combate">
  ...
  function renderResourceSummaryPanel(){
    ...
    return `<article class="card resource-summary-card"><p class="eyebrow">FOCO DO SISTEMA</p><h2>Combate rápido</h2><div class="resource-summary-grid">
      <div class="stat compact-stat" ${themeResourceAttrs('hp')} data-theme-frame="resource"><span>${themeIconSlot('hp')}<span class="theme-resource-label">HP</span></span><strong>${state.currentHp}/${hpMax()}</strong><small>${Math.round((state.currentHp/Math.max(1,hpMax()))*100)}%</small></div>
      <div class="stat compact-stat" ${themeResourceAttrs('en')} data-theme-frame="resource"><span>${themeIconSlot('en')}<span class="theme-resource-label">Energia</span></span><strong>${state.currentEnergy}/${energyMax()}</strong><small>${energy.name}</small></div>
      <div class="stat compact-stat" ${themeResourceAttrs('san')} data-theme-frame="resource"><span>${themeIconSlot('san')}<span class="theme-resource-label">Sanidade</span></span><strong>${state.currentSanity}/100</strong><small>${band.name}</small></div>
      <div class="stat compact-stat" ${themeResourceAttrs('defense')} data-theme-frame="resource"><span>${themeIconSlot('defense')}<span class="theme-resource-label">Defesa base</span></span><strong>${signed(defenseBonus())}</strong><small>DES + armadura + escudo</small></div>
      <div class="stat compact-stat" ${themeResourceAttrs('cast')} data-theme-frame="resource"><span>${themeIconSlot('cast')}<span class="theme-resource-label">Conjuração</span></span><strong>${signed(castAttack())}</strong><small>DT ${castDT()} · ${attrName(g.casting)}</small></div>
      ${hudResources().map(r=>{const key=resourceKey(r);return `<div class="stat compact-stat" ${themeResourceAttrs('divine',r.id||'',key)} data-theme-frame="resource"><span>${themeIconSlot('divine')}<span class="theme-resource-label">${esc(r.name)}</span></span><strong>${resourceValue(r)}/${resourceMaxFor(r)}</strong><small>${r.scope==='collective'?'coletivo':'pessoal'}</small></div>`}).join('')}
  ...
  function renderMiniDeath(){
    ...
    return `<section class="rail-death ${active?'active':''} ${d.dead?'dead':''}" data-theme-component="death">
  ...
  function renderCharacterRail(g){
    ...
    return `<aside class="character-rail card" data-theme-component="profile" data-theme-frame="profile">
      <div class="rail-brandline"><span>LEG · XII · FULMINATA</span><b>${GROUP_LABEL[g.group]||g.group}</b></div>
      <div class="rail-portrait-wrap"><div class="rail-portrait-badges"><label class="rail-orb rail-orb-level" data-theme-component="orb" data-orb="level" data-theme-frame="orb"><span>Nível</span><input id="levelOrbInput" type="number" min="1" max="100" value="${state.level}"></label><div class="rail-orb rail-orb-bp" data-theme-component="orb" data-orb="bp" data-theme-frame="orb"><span>BP</span><strong>${signed(bp())}</strong></div></div><div class="rail-portrait ${h.portraitUrl?'has-image':''}"${h.portraitUrl?` style="background-image:url('${esc(h.portraitUrl)}')"`:''}>${h.portraitUrl?'':esc(initials(state.name||g.name))}</div><button id="heroPortraitUploadBtn" class="rail-photo-button" type="button">Trocar retrato</button></div>
      ...
      <details class="rail-editor"><summary>Editar perfil</summary><div class="stack">
        ...
        <input id="portraitUploadInput" type="file" accept="image/*" hidden>
      </div></details>
  ...
  function renderStatusTab(g){
    return `<section class="tab-pane ${state.activeTab==='status'?'':'hidden'}" data-pane="status" data-theme-zone="status">
      ${renderResourceDock()}
      <section class="overview-grid">
        ...
        <div class="overview-aux overview-side-stack"><div class="overview-talents">${renderTalentsCompact()}</div><div class="overview-quick">${renderStatusQuickCard(g)}<details class="card compact-config"><summary>Ajustes excepcionais</summary><p class="muted compact">Use apenas quando um efeito disser explicitamente que pode ultrapassar o limite normal.</p><div class="grid two compact-fields">${ATTRS.map(([k,a])=>`<label><span class="label">${a} extra</span><input type="number" min="-10" max="10" value="${Number(state.attributeExtras[k])||0}" data-attr-extra="${k}"></label>`).join('')}</div></details><details class="card compact-config"><summary>Diagnóstico mecânico</summary>${renderDiagnostics()}</details></div></div>
      </section>
      <div class="lineage-config-visible">${renderLegacySection()}</div>
    </section>`;
  }
  ...
  function renderEquippedDeck(){
    ...
    const card=(cls,editKey,badge,metaLabels,visual,title,subtitle,description,stats,extra='')=>{
      ...
      return `<article class="equipment-card equipment-card--ornate ${cls}" data-theme-component="equipment-card" data-equipment-kind="${esc(String(cls).split(' ')[0]||'item')}" data-equipment-key="${esc(editKey)}" data-equipment-active="true" data-theme-selectable="true" data-theme-selected="false" data-theme-frame="equipment" tabindex="0" aria-pressed="false"><div class="equipment-card-frame"><div class="equipment-card-topline"><span class="equipment-card-topbadge equipment-card-topbadge--left"><i>${badge.icon}</i><span>${esc(badge.label)}</span></span></div><div class="equipment-card-visual-shell">${visual}<button class="equipment-card-edit" data-edit-equipment="${esc(editKey)}" type="button">Editar</button></div><div class="equipment-card-divider" aria-hidden="true"><span></span></div><div class="equipment-card-copy centered"><h4 class="equipment-card-title" title="${esc(title)}">${esc(title)}</h4>${subtitle?`<div class="equipment-card-subtitle">${esc(subtitle)}</div>`:''}${metas.length?`<div class="equipment-card-meta">${metas.map(m=>`<span class="equipment-card-metabadge">${esc(m)}</span>`).join('')}</div>`:''}${desc?`<p class="equipment-card-description">${esc(desc)}</p>`:''}</div><div class="equipment-card-stats">${stats.map(x=>`<span class="equipment-card-stat">${x}</span>`).join('')}</div>${extra}</div></article>`;
    };
    if(a.equipped){
  ...
  function renderCombatEquipmentSummary(){
    ...
    return `<article class="card equipment-summary-card" data-theme-component="equipment-deck" data-theme-frame="section"><div class="section-title"><div><p class="eyebrow">EQUIPAMENTO EM USO</p><h3>Baralho de combate</h3></div><button type="button" data-go-tab="inventory">Abrir Inventário</button></div>${renderEquippedDeck()}<div class="equipment-inline-summary"><span>${a.equipped?`Armadura +${t.defense} DEF / ${t.reduction} RD`:'Sem armadura'}</span><span>${s.equipped?`Escudo +${shieldDefense()} DEF`:'Sem escudo'}</span><span>${eqWeapons.length?`${eqWeapons.length} arma(s) equipada(s)`:'Sem arma equipada'}</span></div></article>`;
  }
  ...
  function renderCombatTab(fDef,set){
    ...
    return `<section class="tab-pane ${state.activeTab==='combat'?'':'hidden'}" data-pane="combat" data-theme-zone="combat">
  ...
  function magicSacrificeAvailable(k){
    const perAttrMax=Number(magicRules.maxSacrifices||3);
    if(magicRules.sacrificeCanGoBelowZero!==false)return Math.max(0,perAttrMax-magicSacrifice(k));
    return Math.max(0,basePlusLevel(k)-magicSacrifice(k));
  }
  ...
  function bindMagic(){
    ...
    document.querySelectorAll('[data-magic-sac]').forEach(b=>b.onclick=()=>{const [k,ds]=b.dataset.magicSac.split(':'),d=Number(ds),total=magicSacrificeTotal(),cur=magicSacrifice(k),perAttrMax=Number(magicRules.maxSacrifices||3);if(d>0&&(total>=magicRules.maxSacrifices||cur>=perAttrMax))return;state.magic.sacrifices[k]=clamp(cur+d,0,perAttrMax);syncCurrentCaps();save();renderSheet()});
  ...
  function familiarImageSlot(f,index){
  ...
  function renderAbilities(set){
    ...
    return `<section class="abilities-wrap mechanics-section" data-theme-component="abilities"><div class="section-title ability-section-head" data-theme-part="abilities-header"><div><p class="eyebrow">PODERES DIVINOS</p><h2>${lineageAbilityTitle()}</h2><p class="muted compact">As três estacas ficam sempre visíveis. A faixa atual é destacada sem esconder as demais regras.</p></div><div class="row"><span class="pill">${passives.length} passivas</span><span class="pill">${actives.length} ativas</span></div></div><div class="ability-group" data-ability-group="passive"><div class="ability-group-title" data-theme-part="ability-group-title"><b>Passivas</b><span>sempre disponíveis quando a regra permitir</span></div><div class="ability-grid">${passives.length?passives.map(renderAbility).join(''):'<div class="notice">Nenhuma passiva carregada.</div>'}</div></div><div class="ability-group" data-ability-group="active"><div class="ability-group-title" data-theme-part="ability-group-title"><b>Ativas</b><span>nível, custo e uso em combate</span></div><div class="ability-grid">${actives.length?actives.map(renderAbility).join(''):'<div class="notice">Nenhuma ativa carregada.</div>'}</div></div></section>`;
  }
...
function renderStructuredText(text,cls=''){
  ...
  function renderCoreTierRows(tiers,key){
    ...
    return `<div class="core-subtiers">${(tiers||[]).map(t=>`<div class="core-subtier ${coreTierCurrent(t,value)?'current':''}"><b>${esc(t.label||t.id||'Estaca')}</b><div class="core-subtier-copy">${renderStructuredText(t.text||'')}</div></div>`).join('')}</div><div class="core-substake"><div class="row between"><span>Estacas</span><b>${value}/30</b></div><input type="range" min="0" max="30" value="${value}" data-core-stakes="${esc(key)}"><div class="stake-quick"><button data-core-stake-set="${esc(key)}:0">0</button><button data-core-stake-set="${esc(key)}:16">16</button><button data-core-stake-set="${esc(key)}:30">30</button></div></div>`;
  }
...
function renderCoreVariant(a,v,open=false){
  ...
  const ownTiers=(v.tiers||[]).length?`<div class="core-subtiers plain">${(v.tiers||[]).map(t=>`<div class="core-subtier"><b>${esc(t.label||t.id)}</b><div class="core-subtier-copy">${renderStructuredText(t.text||'')}</div></div>`).join('')}</div>`:'';
...
function renderCoreBlocks(a){
  ...
  return `<div class="core-blocks">${blocks.map(b=>{
    ...
    if(b.type==='tiers')return a.tiers?'':`<div class="core-subtiers plain">${(b.items||[]).map(t=>`<div class="core-subtier"><b>${esc(t.label||t.id)}</b><div class="core-subtier-copy">${renderStructuredText(t.text||'')}</div></div>`).join('')}</div>`;
...
function renderAbility(a){
  ...
  const simpleFallback=!tiers.length&&!complex&&!hasCoreNote&&a.extra?`<div class="ability-tier-list single"><div class="ability-tier current"><div class="ability-tier-copy">${renderStructuredText(a.extra)}</div></div></div>`:'';
  return `<article class="ability-card ${locked?'ability-locked':''} ${a.type==='active'?'active-ability':'passive-ability'} ${a.isExtra?'extra-ability':''} ${complex?'ability-complex':''}" data-theme-component="ability" data-ability-id="${esc(a.id||abilityKey(a))}" data-ability-type="${esc(a.type||'passive')}" data-ability-source="${esc(a.sourceGodId||state.godId||'')}" data-theme-frame="ability"><div class="ability-head"><div><div class="row ability-title-row"><b>${esc(a.name)}</b>${a.sourceGodId&&a.sourceGodId!==state.godId?`<span class="pill warn">${godById(a.sourceGodId)?.name||'Legado'}</span>`:''}</div><div class="ability-meta">${a.type==='active'?(a.isExtra?'<span class="meta-extra">Extra</span>':`<span>Nv ${a.level}</span><span>${cost} EN</span>`):'<span>Passiva</span>'}${state.magic?.enabled&&maxSt===29?'<span>mágico · até 29</span>':''}</div></div></div>${summaryHtml}${renderCoreBlocks(a)}${renderAbilityChoices(a)}${renderAbilityResources(a)}${renderAbilityUsage(a)}${tiers.length?`<div class="ability-tier-list">${tiers.map(([key,label,text,current])=>`<div class="ability-tier ${current?'current':''}" data-tier="${key}"><b>${label}</b><div class="ability-tier-copy">${renderStructuredText(text)}</div></div>`).join('')}</div><div class="stake-compact"><div class="row between"><span>Estacas</span><b>${st}/${maxSt}</b></div><input type="range" min="0" max="${maxSt}" value="${st}" data-stakes="${abilityKey(a)}" data-stake-max="${maxSt}"><div class="stake-quick"><button data-stake-set="${abilityKey(a)}:0" data-stake-limit="${maxSt}">0</button><button data-stake-set="${abilityKey(a)}:16" data-stake-limit="${maxSt}">16</button>${maxSt>=30?`<button data-stake-set="${abilityKey(a)}:30" data-stake-limit="${maxSt}">30</button>`:''}</div></div>`:simpleFallback}${a.extra&&a.tiers&&!hasCoreNote?`<details class="ability-notes"><summary>Observações</summary>${renderStructuredText(a.extra)}</details>`:''}${a.type==='active'?`<div class="ability-actions">${a.isExtra?'<span class="pill warn">Habilidade extra</span>':(locked?`<span class="locked">Bloqueada até o nível ${a.level}</span>`:`<button class="primary" data-use-ability="${abilityKey(a)}" data-cost="${cost}">Usar · −${cost} EN</button>`)}${a.isExtra?'':`<small>EN atual ${state.currentEnergy}</small>`}</div>`:''}</article>`;
}
...
function applyManualResource(kind,input){
  ...
  function bindSheet(){
    ...
    // Seleção puramente visual do baralho. Não altera nenhuma mecânica nem é salva no JSON;
    // existe para temas poderem destacar uma carta clicada/focada de forma consistente.
    document.querySelectorAll('[data-theme-selectable="true"]').forEach(card=>{
      const selectCard=()=>{const host=card.closest('[data-theme-component="equipment-deck"]')||document;host.querySelectorAll('[data-theme-selectable="true"]').forEach(other=>{other.dataset.themeSelected='false';other.classList.remove('theme-selected');other.setAttribute('aria-pressed','false')});card.dataset.themeSelected='true';card.classList.add('theme-selected');card.setAttribute('aria-pressed','true')};
      card.addEventListener('click',e=>{if(e.target.closest('button,input,select,textarea,label,a'))return;selectCard()});
      card.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&!e.target.closest('button,input,select,textarea,label,a')){e.preventDefault();selectCard()}});
    });
...
function useAbility(key,cost){
  ...
  function chooseStoredImage(onReady,maxDimension=480,targetLength=150000){
    const input=document.createElement('input');input.type='file';input.accept='image/*';
    input.onchange=()=>{const file=input.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=async()=>{const src=String(reader.result||''),compact=await compactDataImage(src,maxDimension,targetLength);onReady(compact)};reader.onerror=()=>notify('Não foi possível ler essa imagem.');reader.readAsDataURL(file)};input.click();
  }
...
function longRest(){
  ...
  function syncShellChrome(){
    ...
    const paletteLabel=byId('themePaletteLabel'),paletteSwatch=byId('themePaletteSwatch'),specialLabel=byId('themeSpecialLabel'),specialSwatch=byId('themeSpecialSwatch');
    if(paletteLabel)paletteLabel.textContent=`${THEME_PALETTES[palette].label} · ${effectiveMode==='light'?'Light':'Dark'}`;if(paletteSwatch){paletteSwatch.dataset.palette=palette;paletteSwatch.dataset.mode=effectiveMode}if(specialLabel)specialLabel.textContent=exclusiveTheme?exclusiveTheme.name:(exclusiveId?'Tema ausente':special==='fire'?'Fogo':special==='snow'?'Neve':'Nenhum');if(specialSwatch){specialSwatch.textContent=exclusiveTheme?(exclusiveTheme.icon||'✦'):(exclusiveId?'?':special==='fire'?'🔥':special==='snow'?'❄':'✦');specialSwatch.dataset.special=exclusiveTheme?'exclusive':special}
    document.querySelectorAll('[data-palette-choice]').forEach(btn=>{const selected=!exclusiveId&&special==='none'&&btn.dataset.paletteChoice===palette&&(btn.dataset.themeMode||'dark')===mode;btn.classList.toggle('active',selected);btn.setAttribute('aria-selected',selected?'true':'false')});
    document.querySelectorAll('.theme-preset-card').forEach(card=>{const selected=!exclusiveId&&special==='none'&&card.dataset.presetPalette===palette;card.classList.toggle('active',selected)});
  ...
  function bindShellChrome(){
    const themeDialog=byId('themeDialog'),helpDialog=byId('helpDialog'),paletteBtn=byId('themePaletteBtn'),specialBtn=byId('themeSpecialBtn'),upload=byId('bannerUploadInput'),exclusiveInput=byId('exclusiveThemeInput');
    ...
    document.querySelectorAll('[data-palette-choice]').forEach(btn=>btn.onclick=()=>{const palette=btn.dataset.paletteChoice,mode=btn.dataset.themeMode==='light'?'light':'dark';if(!THEME_PALETTES[palette])return;state.appearance.mode=mode;state.appearance.palette=palette;state.appearance.special='none';state.appearance.exclusiveThemeId='';save();syncShellChrome();notify(`Tema ${THEME_PALETTES[palette].label} · ${mode==='light'?'Light':'Dark'} ativado.`)});
    ...
    if(byId('bannerCropSave'))byId('bannerCropSave').onclick=()=>{if(!bannerDraft)return;state.history.bannerSourceUrl=bannerDraft.source;state.history.bannerUrl=bannerDraft.source;state.history.bannerPositionX=bannerDraft.x;state.history.bannerPositionY=bannerDraft.y;state.history.bannerScale=bannerDraft.scale;save();byId('bannerCropDialog')?.close();bannerDraft=null;syncShellChrome();notify('Recorte do banner salvo.')};
    if(byId('bannerCropCancel'))byId('bannerCropCancel').onclick=()=>{byId('bannerCropDialog')?.close();bannerDraft=null};
  }
