...
function renderAbilityChoices(a){
  ...
  return `<div class="ability-choice-zone">${choices.map(choice=>{
    const limit=choiceLimit(a,choice),selected=chosenFor(a,choice),opts=choiceOptions(a,choice),locked=choice.allowReplace===false&&selected.length>=limit;
    return `<section class="ability-choice-block"><div class="row between ability-choice-head"><div><small>ESCOLHA ${choice.mode==='persistent'?'PERSISTENTE':''}</small><b>${esc(choice.label||'Escolha')}</b></div><span class="pill ${selected.length===limit?'good':''}">${selected.length}/${limit}</span></div><div class="ability-choice-options">${opts.map(opt=>{
      ...
      return `<label class="ability-choice-option ${checked?'selected':''} ${disabled?'disabled':''}"><input type="checkbox" data-ability-choice="${esc(abilityKey(a))}|${esc(choice.id)}|${esc(id)}" ${checked?'checked':''} ${disabled?'disabled':''}><span><b>${esc(opt.label||opt.name||id)}</b>${opt.text?`<small>${esc(opt.text)}</small>`:''}${tierText?`<div class="ability-choice-tiertext">${tierText}</div>`:''}</span></label>${checked&&opt.subchoice?`<label class="choice-detail-field"><span class="label">Detalhe da escolha</span><input data-choice-detail="${esc(detailKey)}" value="${esc(detail)}" placeholder="${opt.subchoice.type==='knownAbility'?'Habilidade ou magia escolhida…':'Escolha relacionada…'}"></label>`:''}`;
    }).join('')}</div>${abilityLocked?`<p class="muted choice-lock-note">Disponível a partir do nível ${a.level}.</p>`:(choice.allowReplace===false?'<p class="muted choice-lock-note">Escolha permanente: a ficha não substitui automaticamente uma opção já registrada.</p>':'')}</section>`;
  }).join('')}</div>`;
}
function renderAbilityResources(a){
  const rs=abilityResourcesFor(a);if(!rs.length)return '';
  return `<div class="ability-local-resources">${rs.map(r=>{const key=resourceKey(r),max=resourceMaxFor(r),cur=resourceValue(r);return `<div class="ability-local-resource"><span>${esc(r.name)}</span><div><button type="button" data-divine-resource-adjust="${esc(key)}:-1">−</button><strong>${cur}/${max}</strong><button type="button" data-divine-resource-adjust="${esc(key)}:1">+</button></div></div>`}).join('')}</div>`;
}
function resourceByKey(key){return divineResources().find(r=>resourceKey(r)===key)||null}
  ...
  function targetResourceRows(resource){
    ...
    return rows.map(t=>`<div class="target-resource-row"><input data-target-resource-name="${esc(key)}:${esc(t.id)}" value="${esc(t.name||'Alvo')}" aria-label="Nome do alvo"><div class="target-resource-step"><button type="button" data-target-resource-adjust="${esc(key)}:${esc(t.id)}:-1">−</button><strong>${clamp(Number(t.current)||0,0,max)}/${max}</strong><button type="button" data-target-resource-adjust="${esc(key)}:${esc(t.id)}:1">+</button></div><button type="button" class="danger target-remove" data-target-resource-remove="${esc(key)}:${esc(t.id)}">×</button></div>`).join('');
  }
  function renderTargetResourceTrackers(){
    const rs=targetResources();if(!rs.length)return '';
    return `<section class="target-resource-zone mechanics-section">${rs.map(r=>`<article class="card target-resource-card"><div class="section-title"><div><p class="eyebrow">RECURSO POR ALVO</p><h3>${esc(r.name)}</h3></div><button type="button" data-target-resource-add="${esc(resourceKey(r))}">+ Adicionar alvo</button></div><p class="muted compact">Cada alvo mantém seu próprio contador. Máximo atual: <b>${resourceMaxFor(r)}</b>.</p><div class="target-resource-list">${targetResourceRows(r)||'<div class="notice">Nenhum alvo acompanhado.</div>'}</div></article>`).join('')}</section>`;
  }
