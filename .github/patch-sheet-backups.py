...
version='v5.15.14-backups'
app_path=Path('app-v511.js')
app=app_path.read_text(encoding='utf-8')

storage_anchor="  const STORAGE_KEY = 'duodecima_universal_stage4_v24';\n"
storage_new=storage_anchor + "  const SHEET_BACKUP_DB = 'duodecima_sheet_backups_v1';\n  const SHEET_BACKUP_STORE = 'snapshots';\n  const SHEET_BACKUP_LIMIT = 8;\n  const SHEET_BACKUP_FALLBACK_KEY = 'duodecima_sheet_backup_fallback_v1';\n"
app=replace_one(app,storage_anchor,storage_new,'backup constants')

helpers=r'''  let state = defaultState();
  ...
  function godById'''
app=replace_one(app,"  let state = defaultState();\n\n  function godById",helpers,'backup helpers')

old_load=r'''  function load(){
    try{
...
app=replace_one(app,old_load,new_load,'async load with pre-create backup')

old_handlers=r'''  byId('exportBtn').onclick=()=>{save();const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`duodecima-${(state.name||'personagem').toLowerCase().replace(/[^a-z0-9]+/g,'-')}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)};
  ...
  load();'''
new_handlers=r'''  byId('backupsBtn').onclick=async()=>{await renderBackupDialog();byId('backupDialog').showModal()};
...
app=replace_one(app,old_handlers,new_handlers,'backup UI and destructive handlers')
app_path.write_text(app,encoding='utf-8')

index_path=Path('index.html')
index=index_path.read_text(encoding='utf-8')
old_buttons='''        <button id="exportBtn" class="ghost small-btn" type="button">Exportar JSON</button>\n        <label class="ghost file-btn small-btn">Importar JSON<input id="importInput" type="file" accept="application/json"></label>\n        <button id="helpBtn" class="ghost small-btn" type="button">Ajuda</button>'''
new_buttons='''        <button id="exportBtn" class="ghost small-btn" type="button">Exportar JSON</button>\n        <label class="ghost file-btn small-btn">Importar JSON<input id="importInput" type="file" accept="application/json"></label>\n        <button id="backupsBtn" class="ghost small-btn" type="button">Backups</button>\n        <button id="helpBtn" class="ghost small-btn" type="button">Ajuda</button>'''
index=replace_one(index,old_buttons,new_buttons,'backup toolbar button')
dialog='''  <dialog id="backupDialog" class="theme-dialog">\n    <form method="dialog" class="theme-dialog-card backup-dialog-card">\n      <div class="row between theme-dialog-head"><div><p class="eyebrow">SEGURANÇA</p><h2>Backups da ficha</h2><p class="muted compact">A ficha salva checkpoints automáticos antes de resetar, criar outra ficha ou importar um JSON, além de manter backups periódicos neste navegador.</p></div><button value="cancel" class="theme-dialog-close" aria-label="Fechar">×</button></div>\n      <div class="backup-dialog-actions"><button type="button" id="backupNowBtn" class="primary small-btn">Criar backup agora</button><span class="muted">Os 8 backups automáticos mais recentes são mantidos.</span></div>\n      <div id="backupList" class="backup-list" aria-live="polite"></div>\n      <p class="muted compact backup-local-note">Os backups ficam neste navegador. Para uma cópia fora do dispositivo, continue usando Exportar JSON.</p>\n    </form>\n  </dialog>\n\n'''
index=replace_one(index,'  <dialog id="helpDialog" class="help-dialog">',dialog+'  <dialog id="helpDialog" class="help-dialog">','backup dialog')
index=index.replace('v5.15.13-indefeso',version)
index_path.write_text(index,encoding='utf-8')

styles_path=Path('styles-v511.css')
styles=styles_path.read_text(encoding='utf-8')
marker='/* v5.15.14 · backups locais da ficha */'
if marker not in styles:
    styles += r'''

/* v5.15.14 · backups locais da ficha */
.backup-dialog-card{width:min(760px,calc(100vw - 28px))}
.backup-dialog-actions{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin:4px 0 14px}
.backup-list{display:grid;gap:9px;max-height:min(56vh,520px);overflow:auto;padding-right:3px}
.backup-entry{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:12px 13px;border:1px solid var(--line);border-radius:10px;background:color-mix(in srgb,var(--panel) 96%,var(--accent) 4%)}
.backup-entry-copy{min-width:0;flex:1}.backup-entry-copy strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.backup-entry-copy p{margin:5px 0 0}
.backup-entry .pill{flex:0 0 auto}.backup-empty{padding:18px;border:1px dashed var(--line);border-radius:10px;text-align:center}.backup-empty p{margin:6px 0 0}.backup-local-note{margin:14px 0 0}
@media(max-width:620px){.backup-entry{align-items:flex-start;flex-direction:column}.backup-entry>button{width:100%}.backup-entry-copy{width:100%}}
'''
styles_path.write_text(styles,encoding='utf-8')

bootstrap_path=Path('bootstrap-v511.js')
bootstrap=bootstrap_path.read_text(encoding='utf-8').replace('v5.15.13-indefeso',version)
...
if 'v5.15.14-backups' not in manifest:
    manifest=manifest.replace('\n## Regra de manutenção',note+'\n## Regra de manutenção')
