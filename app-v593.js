(() => {
  ...
  const defaultState = () => ({
    ...
    activeTab:'status',history:{summary:'',goals:'',relationships:'',milestones:'',origin:'',age:'',affiliation:'',description:'',tagline:'',birth:'',residence:'',portraitUrl:'',bannerUrl:'',bannerPositionY:50,bannerScale:100},appearance:{mode:'dark',palette:'red',special:'none'},fortuneBlessing:{enabled:false,tempHp:0},notes:'',
  ...
  function migrate(data){
    ...
    out.appearance.special=out.appearance.special==='fire'?'fire':'none';
    out.fortuneBlessing=Object.assign({enabled:false,tempHp:0},data?.fortuneBlessing||{});
    out.fortuneBlessing.enabled=!!out.fortuneBlessing.enabled;
    out.fortuneBlessing.tempHp=out.fortuneBlessing.enabled?clamp(Number(out.fortuneBlessing.tempHp)||0,0,10):0;
  ...
  function syncCurrentCaps(){
    ...
    state.fortuneBlessing=Object.assign({enabled:false,tempHp:0},state.fortuneBlessing||{});
    state.fortuneBlessing.tempHp=state.fortuneBlessing.enabled?clamp(Number(state.fortuneBlessing.tempHp)||0,0,10):0;
  ...
  function applyHpDelta(delta){
    ...
    if(!delta)return {absorbed:0,hpDelta:0};
    let absorbed=0;
    if(delta<0&&state.fortuneBlessing?.enabled&&(Number(state.fortuneBlessing.tempHp)||0)>0){
      let damage=Math.abs(delta);
      absorbed=Math.min(Number(state.fortuneBlessing.tempHp)||0,damage);
      state.fortuneBlessing.tempHp=clamp((Number(state.fortuneBlessing.tempHp)||0)-absorbed,0,10);
      damage-=absorbed;
      if(damage>0)setHp(state.currentHp-damage);
      return {absorbed,hpDelta:-damage};
    }
    setHp(state.currentHp+delta);
    return {absorbed:0,hpDelta:delta};
  }
  function renderFortuneBlessingCompact(context='status'){
    const f=state.fortuneBlessing||{enabled:false,tempHp:0},temp=clamp(Number(f.tempHp)||0,0,10);
    return `<div class="fortune-temp-hp ${f.enabled?'active':''} fortune-${context}"><label><input type="checkbox" data-fortune-toggle ${f.enabled?'checked':''}><span class="fortune-copy"><b>Bênção de Fortuna</b><em>${f.enabled?'Ativa neste mês':'+10 HP temporários mensais'}</em></span></label><span class="fortune-value">${f.enabled?`${temp}/10 HP TEMP`:'MARCAR'}</span></div>`;
  }
  ...
  function syncShellChrome(){
    ...
    const special=state.appearance?.special==='fire'?'fire':'none';
    const effectiveMode=special==='fire'?'dark':mode;
    ...
    document.documentElement.style.colorScheme=effectiveMode==='light'?'light':'dark';
    if(modeBtn){
      ...
      modeBtn.setAttribute('title',special==='fire'?'Fogo usa modo escuro. Clique para sair do tema especial e ir para Light.':(effectiveMode==='light'?'Mudar para Dark':'Mudar para Light'));
    ...
    if(specialBtn){specialBtn.classList.toggle('active-special',special==='fire');specialBtn.setAttribute('aria-label',`Tema especial atual: ${special==='fire'?'Fogo':'Nenhum'}`)}
    if(specialLabel)specialLabel.textContent=special==='fire'?'Fogo':'Nenhum';
    if(specialSwatch){specialSwatch.textContent=special==='fire'?'🔥':'✦';specialSwatch.classList.toggle('is-fire',special==='fire')}
    ...
    if(bg){
      bg.style.backgroundImage=banner?`url("${banner.replace(/"/g,'\"')}")`:'';
      bg.style.backgroundPosition=`center ${pos}%`;
      bg.style.setProperty('--cover-scale',String((zoom/100).toFixed(2)));
      bg.classList.toggle('has-image',!!banner);
    }
    ...
    if(controls)controls.classList.toggle('hidden',!banner);
    if(posInput)posInput.value=String(pos);
    if(scaleInput)scaleInput.value=String(zoom);
    if(posValue)posValue.textContent=`${pos}%`;
    if(scaleValue)scaleValue.textContent=`${zoom}%`;
  }
  ...
  function renderCombatResourceDock(){
    ...
    return `<section class="combat-resource-dock" aria-label="Recursos de combate">
      ${mini('hp','HP',state.currentHp,hpMax(),renderFortuneBlessingCompact('combat'))}
  ...
  function renderResourceSummaryPanel(){
    ...
    return `<article class="card resource-summary-card"><p class="eyebrow">FOCO DO SISTEMA</p><h2>Combate rápido</h2><div class="resource-summary-grid">
      <div class="stat compact-stat"><span>HP</span><strong>${state.currentHp}/${hpMax()}</strong><small>${state.fortuneBlessing?.enabled?`+ ${state.fortuneBlessing.tempHp||0}/10 temporário`:`${Math.round((state.currentHp/Math.max(1,hpMax()))*100)}%`}</small></div>
  ...
  function renderCharacterRail(g){
    ...
    return `<aside class="character-rail card">
      ...
      ${renderFortuneBlessingCompact('rail')}
      ${renderMiniDeath()}
  ...
  function applyManualResource(kind,input){
    ...
    let hpResult=null;if(kind==='hp')hpResult=applyHpDelta(delta);else if(kind==='en')setEnergy(state.currentEnergy+delta);else if(kind==='san')setSanity(state.currentSanity+delta);else if(kind==='resource')state.resourceCurrent=clamp((state.resourceCurrent||0)+delta,0,resourceMax());
    if(input)input.value='';save();renderSheet();const label=kind==='hp'?'HP':kind==='en'?'Energia':kind==='san'?'Sanidade':god()?.resource?.name||'Recurso';if(kind==='hp'&&hpResult?.absorbed)notify(`Bênção de Fortuna absorveu ${hpResult.absorbed} de dano${hpResult.hpDelta<0?`; ${Math.abs(hpResult.hpDelta)} atingiu o HP normal.`:'.'}`);else notify(`${label} ${delta>0?'+':''}${delta}.`);
  }
  ...
  function bindSheet(){
    ...
    document.querySelectorAll('[data-hp]').forEach(b=>b.onclick=()=>{applyHpDelta(Number(b.dataset.hp));save();renderSheet()});
    document.querySelectorAll('[data-en]').forEach(b=>b.onclick=()=>{setEnergy(state.currentEnergy+Number(b.dataset.en));save();renderSheet()});
    document.querySelectorAll('[data-fortune-toggle]').forEach(el=>el.onchange=()=>{const on=el.checked;state.fortuneBlessing=state.fortuneBlessing||{enabled:false,tempHp:0};state.fortuneBlessing.enabled=on;state.fortuneBlessing.tempHp=on?10:0;save();renderSheet();notify(on?'Bênção de Fortuna ativada: +10 HP temporários.':'Bênção de Fortuna desativada.');});
  ...
  function bindShellChrome(){
    const bannerBtn=byId('bannerBtn'),clear=byId('clearBannerBtn'),help=byId('helpBtn'),helpDialog=byId('helpDialog'),posInput=byId('bannerPositionInput'),scaleInput=byId('bannerScaleInput'),modeBtn=byId('themeModeBtn'),paletteBtn=byId('themePaletteBtn'),specialBtn=byId('themeSpecialBtn'),themeDialog=byId('themeDialog');
    ...
    document.querySelectorAll('[data-special-choice]').forEach(btn=>btn.onclick=()=>{const special=btn.dataset.specialChoice==='fire'?'fire':'none';state.appearance=state.appearance||{mode:'dark',palette:'red',special:'none'};state.appearance.special=special;save();syncShellChrome();notify(special==='fire'?'Tema especial Fogo ativado.':'Tema especial desativado.');});
  }
