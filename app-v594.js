(() => {
  ...
  const defaultState = () => ({
    ...
    activeTab:'status',history:{summary:'',goals:'',relationships:'',milestones:'',origin:'',age:'',affiliation:'',description:'',tagline:'',birth:'',residence:'',portraitUrl:'',bannerUrl:'',bannerPositionY:50,bannerScale:100},appearance:{mode:'dark',palette:'red',special:'none'},fortuneBlessing:{enabled:false,tempHp:0,appliedToHp:false},notes:'',
  ...
  function fortuneHpBonus(){return state.fortuneBlessing?.enabled?10:0}
  function hpMax(){const base=state.exhaustion>=5?Math.max(1,Math.floor(rawHpMax()/2)):rawHpMax();return base+fortuneHpBonus()}
  ...
  function migrate(data){
    ...
    out.fortuneBlessing.appliedToHp=!!out.fortuneBlessing.enabled;
    out.fortuneBlessing.tempHp=0;
  ...
  function syncCurrentCaps(){
    ...
    state.fortuneBlessing=Object.assign({enabled:false,tempHp:0,appliedToHp:false},state.fortuneBlessing||{});
    state.fortuneBlessing.appliedToHp=!!state.fortuneBlessing.enabled;
    state.fortuneBlessing.tempHp=0;
  ...
  function renderFortuneBlessingCompact(context='status'){
    const f=state.fortuneBlessing||{enabled:false,tempHp:0,appliedToHp:false};
    return `<div class="fortune-temp-hp ${f.enabled?'active':''} fortune-${context}"><label><input type="checkbox" data-fortune-toggle ${f.enabled?'checked':''}><span class="fortune-copy"><b>Bênção de Fortuna</b><em>${f.enabled?'Ativa neste mês · +10 no HP atual e máximo':'+10 no HP atual e máximo enquanto ativa'}</em></span></label><span class="fortune-value">${f.enabled?'+10 HP':'MARCAR'}</span></div>`;
  }
  ...
  function renderResourceSummaryPanel(){
    ...
    return `<article class="card resource-summary-card"><p class="eyebrow">FOCO DO SISTEMA</p><h2>Combate rápido</h2><div class="resource-summary-grid">
      <div class="stat compact-stat"><span>HP</span><strong>${state.currentHp}/${hpMax()}</strong><small>${state.fortuneBlessing?.enabled?'Bênção de Fortuna · +10 máximo':`${Math.round((state.currentHp/Math.max(1,hpMax()))*100)}%`}</small></div>
  ...
  function bindSheet(){
    ...
    document.querySelectorAll('[data-fortune-toggle]').forEach(el=>el.onchange=()=>{const on=el.checked;state.fortuneBlessing=state.fortuneBlessing||{enabled:false,tempHp:0,appliedToHp:false};const was=!!state.fortuneBlessing.enabled;if(on===was)return;const current=Number(state.currentHp)||0;state.fortuneBlessing.enabled=on;state.fortuneBlessing.appliedToHp=on;state.fortuneBlessing.tempHp=0;state.currentHp=on?clamp(current+10,0,hpMax()):clamp(current-10,0,hpMax());save();renderSheet();notify(on?'Bênção de Fortuna ativada: +10 no HP atual e máximo.':'Bênção de Fortuna desativada: −10 no HP atual e máximo.');});
