(() => {
  ...
  function migrate(data){
    ...
    out.inventory.items=Array.isArray(data?.inventory?.items)?data.inventory.items.map(it=>Object.assign({id:uid('item'),name:'Item',qty:1,category:'outro',material:'',rune:'',notes:'',imageUrl:'',showInDeck:false,isHeritage:false},it,{qty:Math.max(0,Number(it?.qty)||0),category:normalizeInventoryCategory(it?.category),showInDeck:!!it?.showInDeck,isHeritage:!!(it?.isHeritage || it?.category==='heranca')})):[];
  ...
  function renderSheetHero(g){
    const h=state.history||{}, hasBanner=!!h.bannerUrl;
    const meta=[state.player?`Player: ${esc(state.player)}`:'', state.roma?.cohort?esc(state.roma.cohort):'', h.origin?esc(h.origin):'', h.age?`${esc(h.age)} anos`:'' ].filter(Boolean);
    return `<section class="sheet-head sheet-hero ${hasBanner?'has-banner':''}">
      <div class="sheet-hero-bg"${hasBanner?` style="background-image:url('${esc(h.bannerUrl)}')"`:''}></div>
      <div class="sheet-hero-profile">
        <div class="sheet-hero-copy">
          <p class="eyebrow">LEGIO XII FULMINATA · CHARACTER ARCHIVE</p>
          <h2 class="big-name">${esc(state.name||'Sem nome')}</h2>
          <p class="sheet-hero-tagline">${esc(h.tagline||'Escolha uma imagem horizontal para usar como capa da ficha, como um banner de perfil.')}</p>
          <div class="hero-pills"><span class="pill">Nível ${state.level}</span><span class="pill">BP ${signed(bp())}</span><span class="pill">${esc(g.name)}</span>${meta.map(x=>`<span class="pill">${x}</span>`).join('')}</div>
        </div>
      </div>
      <span class="sheet-hero-mark">XII</span>
    </section>
    <section class="sheet-utility-bar">
      <div class="sheet-utility-info">
        <div class="sheet-utility-card">
          <small>ARQUIVO DA FICHA</small>
          <b>Salvamento local automático</b>
          <span>Importe e exporte o JSON quando quiser fazer backup ou trocar de dispositivo.</span>
        </div>
        <div class="sheet-utility-card banner-status ${hasBanner?'has-thumb':''}">
          <div class="sheet-banner-thumb">${hasBanner?`<img src="${esc(h.bannerUrl)}" alt="Miniatura do banner">`:'<span>sem capa</span>'}</div>
          <div>
            <small>BANNER DO PERSONAGEM</small>
            <b>${hasBanner?'Capa carregada':'Nenhuma capa adicionada'}</b>
            <span>Use uma imagem horizontal para funcionar como capa, estilo banner.</span>
          </div>
        </div>
      </div>
      <div class="sheet-utility-actions">
        <button id="sheetExportBtn" class="ghost" type="button">Exportar JSON</button>
        <button id="sheetImportBtn" class="ghost" type="button">Importar JSON</button>
        <button id="uploadBannerBtn" class="primary" type="button">Alterar banner</button>
        ${hasBanner?'<button id="clearBannerBtn" type="button">Remover banner</button>':''}
        <button id="sheetResetBtn" class="danger" type="button">Resetar</button>
      </div>
    </section>`;
  }
  ...
  function renderStatusTab(g){
    return `<section class="tab-pane ${state.activeTab==='status'?'':'hidden'}" data-pane="status">
      ...
      <section class="overview-grid">
        ...
        <div class="overview-talents overview-side-stack">${renderTalentsCompact()}</div>
  ...
  function renderEquippedDeck(){
    ...
    if(a.equipped){
      ...
      cards.push(`<div class="equipment-card armor"><span class="equipment-card-index">I</span><button class="equipment-card-edit" data-edit-equipment="armor" type="button">Editar</button><div class="equipment-card-topline"><span class="equipment-card-type"><i>${badge.icon}</i>${badge.label}</span></div>${equipmentCardVisual(a.imageUrl,'◈')}<div class="equipment-card-copy centered"><b>${esc(a.name||'Armadura')}</b><span>${esc(t.name)} · ${esc(am.name)}</span>${compactText(a.notes||'',84)?`<p>${esc(compactText(a.notes||'',84))}</p>`:''}</div><div class="equipment-card-stats"><span>DEF +${t.defense}</span><span>RD ${t.reduction}</span></div></div>`);
    }
    if(sh.equipped){
      const badge=equipmentCardLabel('shield');
      cards.push(`<div class="equipment-card shield"><span class="equipment-card-index">II</span><button class="equipment-card-edit" data-edit-equipment="shield" type="button">Editar</button><div class="equipment-card-topline"><span class="equipment-card-type"><i>${badge.icon}</i>${badge.label}</span></div>${equipmentCardVisual(sh.imageUrl,'⬡')}<div class="equipment-card-copy centered"><b>${esc(sh.name||'Escudo')}</b><span>${esc(sm.name)}</span></div><div class="equipment-card-stats"><span>DEF +${shieldDefense()}</span><span>${Number(sh.stakes)||0}/30</span></div></div>`);
    }
    (state.weapons||[]).filter(w=>w.equipped!==false).forEach(w=>{
      const wm=material(w.material),wt=weaponType(w.type),badge=equipmentCardLabel('weapon',w.type);
      cards.push(`<div class="equipment-card weapon"><span class="equipment-card-index">${cards.length+1}</span><button class="equipment-card-edit" data-edit-equipment="weapon:${w.id}" type="button">Editar</button><div class="equipment-card-topline"><span class="equipment-card-type"><i>${badge.icon}</i>${badge.label}</span></div>${equipmentCardVisual(w.imageUrl,'✦')}<div class="equipment-card-copy centered"><b>${esc(w.name||'Arma')}</b><span>${esc(wt.name)} · ${esc(wm.name)}</span></div><div class="equipment-card-stats"><span>ATQ ${signed(weaponAttack(w))}</span><span>${weaponDamageFormula(w)}</span></div></div>`);
    });
    (state.inventory?.items||[]).filter(it=>it.showInDeck).forEach(it=>{
      const badge=equipmentCardLabel('item',it.category),desc=compactText(it.notes||it.rune||'',92);
      cards.push(`<div class="equipment-card item"><span class="equipment-card-index">${cards.length+1}</span><button class="equipment-card-edit" data-edit-equipment="item:${it.id}" type="button">Editar</button><div class="equipment-card-topline"><span class="equipment-card-type"><i>${badge.icon}</i>${badge.label}</span>${it.isHeritage?'<span class="equipment-card-heritage">Herança</span>':''}</div>${equipmentCardVisual(it.imageUrl,'✧')}<div class="equipment-card-copy centered"><b>${esc(it.name||'Item')}</b><span>${it.material?esc(it.material):'Item do acervo'}</span>${desc?`<p>${esc(desc)}</p>`:''}</div><div class="equipment-card-stats"><span>QTD ${Math.max(0,Number(it.qty)||0)}</span>${it.rune?`<span>${esc(compactText(it.rune,26))}</span>`:''}</div></div>`);
    });
    const desired=Math.max(4,cards.length);
    for(let i=cards.length;i<desired;i++)cards.push(`<div class="equipment-card empty"><span class="equipment-card-index">${i+1}</span><button class="equipment-card-edit" data-go-tab="inventory" type="button">Adicionar</button><div class="equipment-card-topline"><span class="equipment-card-type"><i>＋</i>SLOT LIVRE</span></div><div class="equipment-card-glyph">XII</div><div class="equipment-card-copy centered"><b>Item ou equipamento</b><span>Equipe uma arma ou fixe um item do inventário</span></div></div>`);
  ...
  function bindInventory(){
    ...
    const add=byId('addInventoryItem');if(add)add.onclick=()=>{inv.items.push({id:uid('item'),name:'Novo item',qty:1,category:'outro',material:'',rune:'',notes:'',imageUrl:'',showInDeck:false,isHeritage:false});save();renderSheet()};
    ...
    document.querySelectorAll('[data-consume-item]').forEach(b=>b.onclick=()=>{const it=inv.items.find(x=>x.id===b.dataset.consumeItem);if(!it)return;const qty=Math.max(0,Number(it.qty)||0);if(qty<=0)return;if(qty<=1){inv.items=inv.items.filter(x=>x.id!==it.id);notify('Consumível removido do inventário.');}else{it.qty=qty-1;notify(`Consumível usado: ${it.name||'item'} · ${it.qty} restante(s).`);}save();renderSheet()});
  ...
  function renderSheet(){
    ...
    sheetView.innerHTML=`<div class="sheet-frame">${renderCharacterRail(g)}<div class="sheet-main">${renderSheetHero(g)}${renderTabs()}${renderStatusTab(g)}${renderCombatTab(fDef,set)}${renderInventoryTab()}${renderFamiliarsTab()}${renderRomaTab()}${renderMagicTab()}${renderHistoryTab()}${renderNotesTab()}</div></div>`;
    bindSheet();save();
  }
  ...
  function bindSheet(){
    ...
    const clearBanner=byId('clearBannerBtn'); if(clearBanner) clearBanner.onclick=()=>{state.history.bannerUrl='';save();renderSheet();notify('Banner removido.')};
    const sheetExport=byId('sheetExportBtn'); if(sheetExport) sheetExport.onclick=()=>byId('exportBtn')?.click();
    const sheetImport=byId('sheetImportBtn'); if(sheetImport) sheetImport.onclick=()=>byId('importInput')?.click();
    const sheetReset=byId('sheetResetBtn'); if(sheetReset) sheetReset.onclick=()=>byId('resetBtn')?.click();
