(() => {
  ...
  function renderEquippedDeck(){
    ...
    (state.inventory?.items||[]).filter(it=>it.showInDeck).forEach(it=>{const badge=equipmentCardLabel('item',it.category),effect=compactText(it.effect||it.rune||it.notes||'',86),isWeapon=it.category==='arma-corpo'||it.category==='arma-distancia';pushCard(`<article class="equipment-card item ${it.category}" tabindex="0"><div class="equipment-card-topline"><span class="equipment-card-type"><i>${badge.icon}</i>${esc(inventoryCategoryLabel(it.category).replace(/^Arma \(|\)$/g,''))}</span>${it.isHeritage?'<span class="equipment-card-heritage">HERANÇA</span>':''}</div><button class="equipment-card-edit" data-edit-equipment="item:${it.id}" type="button">Editar</button>${equipmentCardVisual(it.imageUrl,'✧')}<div class="equipment-card-copy centered"><b>${esc(it.name||'Item')}</b><span>${isWeapon?`${it.attackAttr==='des'?'Destreza':'Força'}${it.material?` · ${esc(it.material)}`:''}`:(it.material?esc(it.material):inventoryCategoryLabel(it.category))}</span>${effect?`<p>${esc(effect)}</p>`:''}</div><div class="equipment-card-stats">${isWeapon?`<span>ATQ ${signed(Number(it.attackBonus)||0)}</span><span>${esc(it.damage||'dano livre')}</span>`:`<span>QTD ${Math.max(0,Number(it.qty)||0)}</span><span>${it.effect?esc(compactText(it.effect,28)):'consulta rápida'}</span>`}</div>${it.category==='consumivel'?`<button class="equipment-consume-btn" data-consume-item="${it.id}" ${Math.max(0,Number(it.qty)||0)<=0?'disabled':''}>Consumir 1</button>`:''}</article>`)});
  ...
  function bindShellChrome(){
    const bannerBtn=byId('bannerBtn'),clear=byId('clearBannerBtn'),help=byId('helpBtn'),dialog=byId('helpDialog');
    if(bannerBtn)bannerBtn.onclick=()=>chooseStoredImage(url=>{state.history.bannerUrl=url;save();syncShellChrome();render();notify('Banner atualizado.')});
    if(clear)clear.onclick=()=>{state.history.bannerUrl='';save();syncShellChrome();render();notify('Banner removido.')};
